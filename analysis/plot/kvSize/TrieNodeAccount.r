...
if (T) {
    ...
    ggplot(data = x1_filtered, aes(x = bucket, y = count)) +
        ...
        geom_text(
            data = vline_labels,
            aes(x = xintercept, y = max(x1$count) + 4200000, label = label),
        ...
        coord_cartesian(ylim = c(0, max(x1$count) + 3000000), xlim = c(0, 630)) +
        scale_x_continuous(expand = c(0, 0), breaks = c(0, 200, 400, 600), labels = format(c(0, 200, 400, 600), scientific = FALSE)) +
        scale_y_continuous(expand = c(0, 0), breaks = seq(0, 80000000, 40000000), labels = format(seq(0, 80, 40), scientific = FALSE)) +
        ylab("Count (M)") +
        xlab("KV size (byte)") +
        theme_bw() +
        theme(
