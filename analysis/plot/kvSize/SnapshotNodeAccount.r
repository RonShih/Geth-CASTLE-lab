...
if (T) {
    ...
    ggplot(data = x1_filtered, aes(x = bucket, y = count)) +
        ...
        geom_text(
            data = vline_labels,
            aes(x = xintercept, y = max(x1$count) + 14000000, label = label),
        ...
        coord_cartesian(ylim = c(0, max(x1$count) + 10000000), xlim = c(0,157.5)) +
        scale_x_continuous(expand = c(0, 0), breaks = c(0, 50, 100, 150), labels = format(c(0, 50, 100, 150), scientific = FALSE)) +
        scale_y_continuous(expand = c(0, 0), breaks = seq(0, 100000000, 50000000), labels = format(seq(0, 100, 50), scientific = FALSE)) +
        ylab("Count (M)") +
        xlab("KV size (byte)") +
        theme_bw() +
        theme(
